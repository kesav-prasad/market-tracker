...
async function run() {
  ...
  const mockSymbols = ['NIFTYBEES', 'SENSEXBETA', 'BANKBEES', 'GOLDBEES', 'SILVERBEES', 'POWERINDIA'];
  
  const origFindMany = prisma.instrument.findMany.bind(prisma.instrument);
  prisma.instrument.findMany = async (args) => {
      return origFindMany({ where: { symbol: { in: mockSymbols } } });
  };
  
  const service = new DataIngestionService();
  
  for (const date of dates) {
    console.log(`Fast-forwarding mock instruments to ${date.toISOString()}`);
    await service.ingestDataForDate(date);
  }
  console.log("Mock instruments are fully up to date!");
}
run();
