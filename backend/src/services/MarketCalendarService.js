...
export class MarketCalendarService {
     ...
     */
    static async isHoliday(date, calendarId = 1) {
     ...
     */
    static async isValidTradingSession(date, calendarId = 1) {
        const { toZonedTime } = require('date-fns-tz');
        const zoned = toZonedTime(date, 'Asia/Kolkata');
        const start = new Date(Date.UTC(zoned.getFullYear(), zoned.getMonth(), zoned.getDate()));
        ...
        if (session) {
            if (session.specialSessionFlag || session.isTradingDay)
                return true;
            if (session.status === 'HOLIDAY' || !session.isTradingDay)
                return false;
        }
        if (isWeekend(date)) {
            return false;
     ...
     */
    static async getPreviousTradingSession(date, calendarId = 1) {
        const { toZonedTime } = require('date-fns-tz');
     ...
     */
    static async getNextTradingSession(date, calendarId = 1) {
        const { toZonedTime } = require('date-fns-tz');
        const zoned = toZonedTime(date, 'Asia/Kolkata');
     ...
     */
    static async generateTradingSessions(start, end, calendarId = 1) {
        const sessions = [];
     ...
     */
    static async getExpiryDateForMonth(year, month, calendarId = 1) {
